...
def _first_gt_bboxes(video_path: str) -> List[Tuple[float, float, float, float]]:
    ...
    raw_boxes = frames.get(first_idx, [])
...
def _select_primary_bbox(boxes: Sequence[Tuple[float, float, float, float]]) -> Optional[Tuple[float, float, float, float]]:
    if not boxes:
        return None
    return boxes[0]
...
class TaMOsTracker(TrackingModel):
    ...
    def predict(self, video_path: str) -> List[FramePrediction]:
        ...
        init_boxes = _first_gt_bboxes(video_path)
        primary_init = _select_primary_bbox(init_boxes)
        if not _is_valid_bbox(primary_init):
            cap.release()
            raise RuntimeError(
                "TaMOs requires a valid initial ground-truth bounding box. "

...
class _PickleSubjectClassifier(SubjectClassifier):
    ...
    def _get_corr_keep_indices(self) -> Optional[np.ndarray]:
        if self._tabular_pipeline is None:
            return None
        named_steps = getattr(self._tabular_pipeline, "named_steps", None)
        if not isinstance(named_steps, dict):
            return None
        corr_step = named_steps.get("corr")
        keep_indices = getattr(corr_step, "keep_indices_", None)
        if keep_indices is None:
            return None
        arr = np.asarray(keep_indices, dtype=np.int64).reshape(-1)
        if arr.size == 0:
            return None
        return arr
    ...
    def _build_feature_importance_summary(
        ...
        values = np.asarray(raw_importances, dtype=np.float64).reshape(-1)
        out: Dict[str, Any] = {
            "feature_importances": values.astype(float).tolist(),
        }

        n_input = max(0, int(n_input_features))
        n_model = int(values.shape[0])
        out["feature_importances_meta"] = {
            "n_input_features": n_input,
            "n_model_features": n_model,
            "aligned_to_input": bool(n_input > 0 and n_input == n_model),
        }

        if n_input <= 0 or n_model == 0:
            return out
        if n_model == n_input:
            return out

        keep_indices = self._get_corr_keep_indices()
        if (
            keep_indices is not None
            and keep_indices.shape[0] == n_model
            and int(np.max(keep_indices)) < n_input
        ):
            full = np.zeros((n_input,), dtype=np.float64)
            full[keep_indices] = values
            out["feature_importances"] = full.astype(float).tolist()
            out["feature_importances_reduced"] = values.astype(float).tolist()
            out["feature_importances_keep_indices"] = keep_indices.astype(int).tolist()
            dropped = np.setdiff1d(np.arange(n_input, dtype=np.int64), keep_indices, assume_unique=False)
            out["feature_importances_dropped_indices"] = dropped.astype(int).tolist()
            out["feature_importances_meta"]["aligned_to_input"] = True
            out["feature_importances_meta"]["alignment_method"] = "correlation_filter_keep_indices"
            return out

        out["feature_importances_meta"]["alignment_method"] = "unavailable"
        out["feature_importances_meta"]["note"] = (
            "Model importances are reported in transformed feature space; "
            "unable to map back to original feature keys."
        )
        return out

...
@register_feature_extractor("DELETED_motion_only")
class _LegacyMotionOnlyFeatureExtractor(TrajectoryFeatureExtractor):
    name = "LegacyMotionOnlyFeatures"
    ...
    def __init__(self, params: Optional[Dict[str, Any]] = None):
        ...
        self._subject_stats = list(cfg.get("aggregate_stats", ["mean", "std", "min", "max"]))
    ...
    def extract_video(
        ...
        feat = _compute_motion_features(samples)
        return OrderedDict((k, float(feat.get(k, 0.0))) for k in self._video_keys)
...
@register_feature_extractor("DELETED_motion_texture")
class _LegacyMotionTextureFeatureExtractor(TrajectoryFeatureExtractor):
    name = "LegacyMotionTextureFeatures"
    DEFAULT_CONFIG: Dict[str, Any] = {
        "aggregate_stats": ["mean", "std", "min", "max"],
        "texture_patch_size": 64,
        "texture_pca_dim": 16,
    }
    ...
    def __init__(self, params: Optional[Dict[str, Any]] = None):
        cfg = {**self.DEFAULT_CONFIG, **(params or {})}
        self._subject_stats = list(cfg.get("aggregate_stats", ["mean", "std", "min", "max"]))
        self._patch_size = int(cfg.get("texture_patch_size", 64))
        self._texture_pca_dim = int(cfg.get("texture_pca_dim", 16))
        self._texture_keys = [f"tex_pca_{i:02d}" for i in range(self._texture_pca_dim)]

        self._video_keys = list(MOTION_FEATURE_KEYS) + list(CSA_FEATURE_KEYS) + self._texture_keys
        self._subject_keys = _build_subject_keys(self._video_keys, self._subject_stats)
    ...
    def _pca_fit_transform(X: np.ndarray, target_dim: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if d <= target_dim:
            ...
            return reduced, np.zeros((d,), dtype=np.float64), np.eye(target_dim, d, dtype=np.float64)
        ...
        if reduced.shape[1] < target_dim:
            reduced = np.hstack([
                reduced,
                np.zeros((n, target_dim - reduced.shape[1]), dtype=reduced.dtype),
            ])
    ...
    def _extract_raw_texture_vec(
        self,
        samples: Sequence[FramePrediction],
        video_path: Optional[str],
    ) -> List[float]:
        n_dim = len(_RAW_TEXTURE_KEYS_PER_FRAME) * 2
        if not samples or not video_path:
            return [0.0] * n_dim

        key_samples = [samples[0], samples[-1] if len(samples) > 1 else samples[0]]
        rows: List[List[float]] = []
        for s in key_samples:
            patch = _read_segmented_patch(video_path, s, target_size=self._patch_size)
            if patch is None:
                rows.append([0.0] * len(_RAW_TEXTURE_KEYS_PER_FRAME))
                continue
            tex = _extract_texture_from_patch(patch)
            rows.append([float(tex.get(k, 0.0)) for k in _RAW_TEXTURE_KEYS_PER_FRAME])

        flat = np.asarray(rows, dtype=np.float64).reshape(-1)
    ...
    def extract_video(
        ...
        raw_vec = self._extract_raw_texture_vec(samples, video_path)

        out: Dict[str, float] = OrderedDict()
        for k in MOTION_FEATURE_KEYS:
            out[k] = float(motion.get(k, 0.0))
        for k in CSA_FEATURE_KEYS:
            out[k] = float(csa.get(k, 0.0))
        for k in self._texture_keys:
            out[k] = 0.0
        out["_raw_texture_vec"] = raw_vec
        return out
    ...
    def finalize_batch(
        ...
        *,
        ...
        indices: List[int] = []
        for i, feat in enumerate(features_list):
            raw = feat.get("_raw_texture_vec")
            if raw is None:
                continue
            raw_rows.append(np.asarray(raw, dtype=np.float64).reshape(-1))
            indices.append(i)
        ...
        if raw_rows:
            ...
            if fit:
                reduced, self._pca_mean, self._pca_components = self._pca_fit_transform(X, self._texture_pca_dim)
            else:
                if self._pca_mean is None or self._pca_components is None:
                    raise RuntimeError(
                        "Legacy motion_texture PCA state is not fitted; run finalize_batch(..., fit=True) first."
                    )
                reduced = self._pca_transform(X, self._pca_mean, self._pca_components, self._texture_pca_dim)

        idx_map = {src: j for j, src in enumerate(indices)}
        ...
        for i, feat in enumerate(features_list):
            ...
            if reduced is not None and i in idx_map:
                tex = reduced[idx_map[i]]
            else:
                tex = np.zeros((self._texture_pca_dim,), dtype=np.float64)
            for j, k in enumerate(self._texture_keys):
                out[k] = float(tex[j]) if j < tex.shape[0] else 0.0
            out_list.append(out)
        return out_list

...
def _torch_load_with_compat(path, *, map_location=None, strict_weights_only=False):
    """Load checkpoints across PyTorch versions.

    PyTorch 2.6 changed the default of ``weights_only`` to ``True`` which breaks legacy
    checkpoints that serialize constructors. We explicitly set ``weights_only=False``
    when available, while keeping compatibility with older torch versions that do not
    accept the argument.
    """

    if strict_weights_only:
        # Explicit escape hatch if a future call actually wants weights_only=True.
...
def load_weights(net, path, strict=True):
    checkpoint_dict = _torch_load_with_compat(path)
...
def torch_load_legacy(path):
    ...
    # Load network
    checkpoint_dict = _torch_load_with_compat(path, map_location='cpu')

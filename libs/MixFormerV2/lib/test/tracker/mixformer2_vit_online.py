...
class MixFormerOnline(BaseTracker):
    ...
    def track(self, image, info: dict = None):
        ...
        if not torch.is_tensor(pred_box_tensor):
            pred_box_tensor = torch.as_tensor(pred_box_tensor, dtype=pred_boxes.dtype, device=pred_boxes.device)
        pred_box = pred_box_tensor.tolist()
        ...
        if self.save_all_boxes:
            ...
            if not torch.is_tensor(all_boxes):
                all_boxes = torch.as_tensor(all_boxes, dtype=pred_boxes.dtype, device=pred_boxes.device)

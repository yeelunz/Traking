...
OUT_DIR = RESULT_DIR / "analysis" / "confidence_center_error_audit" / "n003"
...
def _corr_stats(scores: np.ndarray, center_errors: np.ndarray) -> CorrResult:
    pearson = _pearson(scores, center_errors)
    spearman = _spearman(scores, center_errors)
    slope, intercept = _linreg(scores, center_errors)
    p_pearson = _perm_pvalue(scores, center_errors, _pearson)
    p_spearman = _perm_pvalue(scores, center_errors, _spearman)
    return CorrResult(
        n=int(len(scores)),
        pearson_r=pearson,
...
def _bin_summary(scores: np.ndarray, center_errors: np.ndarray, n_bins: int = 10) -> list[dict[str, Any]]:
    edges = _quantile_bins(scores, n_bins)
    out: list[dict[str, Any]] = []
    global_median = float(np.median(center_errors))
        ...
        out.append(
            {
                ...
                "score_hi": hi,
                "n": int(sel.sum()),
                "ce_mean": float(np.mean(vals)),
                "ce_median": float(np.median(vals)),
                "ce_std": float(np.std(vals, ddof=0)),
                "ce_ci95_lo": None if ci is None else float(ci[0]),
                "ce_ci95_hi": None if ci is None else float(ci[1]),
                "ce_p90": float(np.quantile(vals, 0.9)),
                "high_error_rate_ce_gt_global_median": float(np.mean(vals > global_median)),
            }
        )
    return out
...
def main() -> None:
    ...
    table_csv = OUT_DIR / "confidence_center_error_table.csv"
    ...
    ce_all = np.array([float(r["ce"]) for r in rows], dtype=np.float64)
    ...
    ce_scored = np.array([float(r["ce"]) for r in scored_rows], dtype=np.float64)

    corr_all_scored = _corr_stats(scores, ce_scored)
    bins_all_scored = _bin_summary(scores, ce_scored, n_bins=10)

    q20 = float(np.quantile(scores, 0.2))
    q80 = float(np.quantile(scores, 0.8))
    low = ce_scored[scores <= q20]
    high = ce_scored[scores >= q80]
    low_high_summary = {
        "q20": q20,
        "q80": q80,
        "n_low": int(len(low)),
        "n_high": int(len(high)),
        "ce_low_mean": float(np.mean(low)) if len(low) else None,
        "ce_high_mean": float(np.mean(high)) if len(high) else None,
        "delta_low_minus_high": float(np.mean(low) - np.mean(high)) if len(low) and len(high) else None,
        "delta_ci95": (
            None if len(low) < 2 or len(high) < 2 else _bootstrap_ci_diff_mean(low, high)
        ),
        "perm_p": None if len(low) < 2 or len(high) < 2 else _perm_test_diff_mean(low, high),
    }

    fb = np.array([float(r["ce"]) for r in rows if r["fallback"]], dtype=np.float64)
    nfb = np.array([float(r["ce"]) for r in rows if not r["fallback"]], dtype=np.float64)
    fallback_cmp = {
        "n_fallback": int(len(fb)),
        "n_non_fallback": int(len(nfb)),
        "ce_fallback_mean": float(np.mean(fb)) if len(fb) else None,
        "ce_non_fallback_mean": float(np.mean(nfb)) if len(nfb) else None,
        "delta_fallback_minus_non_fallback": float(np.mean(fb) - np.mean(nfb)) if len(fb) and len(nfb) else None,
        "delta_ci95": (
            None if len(fb) < 2 or len(nfb) < 2 else _bootstrap_ci_diff_mean(fb, nfb)
        ),
        "perm_p": None if len(fb) < 2 or len(nfb) < 2 else _perm_test_diff_mean(fb, nfb),
    }
    ...
    for v in videos:
        ...
        vy = np.array([float(r["ce"]) for r in v_rows], dtype=np.float64)
        per_video[v] = {
    ...
    summary = {
        ...
        "overall": {
            "ce_mean_all_rows": float(np.mean(ce_all)),
            "ce_mean_scored_rows": float(np.mean(ce_scored)),
            "corr_scored": asdict(corr_all_scored),
            "score_ce_bins_scored": bins_all_scored,
    ...
    summary_json = OUT_DIR / "confidence_center_error_summary.json"
    summary_json.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")

    md_lines: list[str] = []
    md_lines.append("# Confidence vs Center Error Statistical Audit (n003)")
    md_lines.append("")
    ...
    md_lines.append(f"- Linear slope (CE~score): {corr_all_scored.slope}")
    md_lines.append("")
    md_lines.append("## 20% Low vs 20% High Confidence")
    md_lines.append(f"- q20={q20}, q80={q80}")
    md_lines.append(f"- mean CE low={low_high_summary['ce_low_mean']}, high={low_high_summary['ce_high_mean']}")
    md_lines.append(f"- delta(low-high)={low_high_summary['delta_low_minus_high']}")
    ...
    md_lines.append(
        f"- mean CE fallback={fallback_cmp['ce_fallback_mean']}, non-fallback={fallback_cmp['ce_non_fallback_mean']}"
    )
    md_lines.append(
        f"- delta(fallback - non-fallback)={fallback_cmp['delta_fallback_minus_non_fallback']}"
    )
    md_lines.append(f"- permutation p={fallback_cmp['perm_p']}")

    (OUT_DIR / "confidence_center_error_report.md").write_text("\n".join(md_lines), encoding="utf-8")

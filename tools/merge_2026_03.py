#!/usr/bin/env python
"""將 dataset/2026_03/ 的資料合併到 dataset/merged_extend/。

結構說明：
  2026_03/c2_Image05伸握伸.wmv   → merged_extend/c2/Image05伸握伸.wmv
  2026_03/c2_Image05伸握伸.json  → merged_extend/c2/Image05伸握伸.json  (mask_path 更新)
  2026_03/c2_Image05伸握伸/      → merged_extend/c2/seg_masks/Image05伸握伸/  (PNG 遮罩)

受試者標籤：
  c2 = 1  (術後 CTS，post-surgery)
  c4 = 0  (正常組)

執行方式：
    python tools/merge_2026_03.py [--dry-run]
"""
...
SOURCE_DIR = DATASET_DIR / "2026_03"
TARGET_DIR = DATASET_DIR / "merged_extend"

# 從檔名解析 subject 前綴與 video stem
# e.g. "c2_Image05伸握伸.wmv" → subject="c2", stem="Image05伸握伸"
...
def _process_json(
    ...
    dst_json: Path,
    subject: str,
    orig_stem: str,   # e.g. "c2_Image05伸握伸"
    video_stem: str,  # e.g. "Image05伸握伸"
    dry_run: bool,
) -> int:
    ...
    #   原始: "seg_masks/c2_Image05伸握伸/frame_00000001.png"
    #   目標: "seg_masks/Image05伸握伸/frame_00000001.png"   (相對於 c2/ 資料夾)
...
def _copy_seg_masks(
    src_frame_dir: Path,   # e.g. 2026_03/c2_Image05伸握伸/
    dst_masks_dir: Path,   # e.g. merged_extend/c2/seg_masks/Image05伸握伸/
    dry_run: bool,
) -> int:
    """把 frame PNG 複製為 seg_masks。回傳複製數量。"""
    count = 0
    if not src_frame_dir.exists():
        print(f"  [警告] 遮罩來源不存在: {src_frame_dir}")
        return 0
    for png in sorted(src_frame_dir.glob("*.png")):
...
def _update_ann_txt(target_dir: Path, new_labels: dict[str, int], notes: dict[str, str], dry_run: bool) -> None:
...
def merge(dry_run: bool = False) -> None:
    ...
    # --- 收集 videos ---
    subjects_processed: dict[str, list[str]] = {}  # subject → [video stems]
    ...
    for wmv in sorted(SOURCE_DIR.glob("*.wmv")):
        ...
        subject = m.group(1)   # e.g. "c2"
        orig_stem = wmv.stem   # e.g. "c2_Image05伸握伸"
        video_stem = m.group(2)  # e.g. "Image05伸握伸"
        ...
        # 3. 複製 seg_masks (來自 frame 子目錄)
        src_mask_dir = SOURCE_DIR / orig_stem  # e.g. 2026_03/c2_Image05伸握伸/
        ...
        if src_labels.exists():
            dst_labels = dst_subject / "labels.txt"
            if not dry_run:
                _copy(src_labels, dst_labels)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="合併 2026_03 資料集")
